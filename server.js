...
// Маршрут для получения данных (с фильтрацией по датам)
app.get("/data", async (req, res) => {
  try {
    const { startDate, endDate } = req.query;

    // Если startDate и endDate не указаны, установим диапазон по умолчанию
    const start = startDate || '2024-09-01';
    const end = endDate || '2024-09-16';
    ...
    const query = `
      ...
      INNER JOIN (
        SELECT path FROM mm.naz_type_dir nd WHERE 'GROUP_CONS' = ANY(tags)
